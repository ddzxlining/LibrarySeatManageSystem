...
namespace SelectOnWeb.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            var result = await SignInManager.PasswordSignInAsync(model.Sno, model.Password, model.RememberMe, shouldLockout: false);
            var user = UserManager.FindByName(model.Sno);
            ViewData["name"] = "未登录";
            switch (result)
            {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser { UserName = model.Sno, Email = model.Email,Name=model.Name };
        ...
        #region+批量注册用户。有单元测试中的方法调用。项目发布时为注释状态；

        //[HttpPost]
        [AllowAnonymous]
        public async void Registers(RegisterViewModel model)
        {
